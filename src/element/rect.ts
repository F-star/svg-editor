...
export class Rect extends FElement {
  ...
  constructor(el: SVGRectElement)
  constructor(x: number | SVGRectElement, y?: number, w?: number, h?: number) {
    super()
    if (typeof x == 'object') {
      this.el_ = x
    } else {
      this.el_ = document.createElementNS(NS.SVG, 'rect') as SVGRectElement
      this.setAttr('x', x + '')
      this.setAttr('y', y + '')
